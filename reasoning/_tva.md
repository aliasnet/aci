---
key: _tva
description: TVA is core semantic logic engine for a self-evaluating control loop for reasoning, where each step checks how well we aligns with the intended goal and then adapts behavior
---

[TVA :: Truth + Validation = Alignment]
Semantic text-only, tool-agnostic, loads pre-response, works across latent space, per-node context to thought/output, enable _metacognition.
[Similarity / Tension]
