...
...
→ READ (if present): 
`README.md`, `sanity.md`, `entities.json`, `functions.json`, `connectors/github_connector.json`
...
- If force/override is ambiguous → **abstain** and report via readiness JSON.

SELF-IMPROVEMENT
- Append a brief `<lessons>` note in `notes` after each action.
- Prefer calibration/lint over refactors when uncertain.

READINESS REPORT JSON (always reply AFTER EVERY action, success or not)
